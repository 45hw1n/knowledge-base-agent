...
export function InvoiceDetail({ invoice }: { invoice: Invoice }) {
  const linkedPayments = paymentsMock.filter((payment) => payment.invoiceId === invoice.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <ConversationThread messages={invoice.conversation} />
      </div>

      <SourceFooter sourceUrl={invoice.sourceUrl} createdAt={invoice.createdAt} updatedAt={invoice.updatedAt} />
    </div>
  );
}
